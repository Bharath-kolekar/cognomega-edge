...
function getCallerEmail(req: Request): string | null {
  ...
  const fromJwt = unsafeDecodeJwtEmail(bearerTok) || unsafeDecodeJwtEmail(cookieTok) || "";
...
async function getBalance(env: AuthBillingEnv, email: string): Promise<BalanceRow> {
...
async function setBalance(env: AuthBillingEnv, email: string, newBalance: number): Promise<BalanceRow> {
...
async function adjustBalance(env: AuthBillingEnv, email: string, delta: number): Promise<BalanceRow> {
...
function pickGroqModel(env: AuthBillingEnv)   { return env.GROQ_MODEL   || "llama-3.1-8b-instant"; }
function pickCfModel(env: AuthBillingEnv)     { return env.CF_AI_MODEL  || "@cf/meta/llama-3.1-8b-instruct"; }
function pickOpenAiModel(env: AuthBillingEnv) { return env.OPENAI_MODEL || "gpt-4o-mini"; }
function groqBase(env: AuthBillingEnv)  { return env.GROQ_BASE  || "https://api.groq.com/openai/v1"; }
function openAiBase(env: AuthBillingEnv){ return env.OPENAI_BASE|| "https://api.openai.com/v1"; }
...
function revTs(ts: number) { const v = Math.max(0, Math.min(Number.MAX_SAFE_INTEGER, ts)); return v ? (Number.MAX_SAFE_INTEGER - v).toString(36) : "0"; }
...
async function createJob(env: AuthBillingEnv, email: string, type: string, params?: Record<string, unknown>): Promise<Job> {
  const ts = Date.now();
  const job: Job = {
...
export async function handleAuthBilling(
  ...
  if (p === "/ready" || p === "/api/ready" || p === "/healthz" || p === "/api/healthz" || p === "/api/v1/healthz") {
    return null;
  }
  ...
  try {
    ...
    if (p === "/auth/guest" || p === "/api/auth/guest" || p === "/api/v1/auth/guest") {
      if (req.method !== "POST") return toJson({ error: "method_not_allowed" }, req, env, 405);
    ...
    if (p === "/api/credits" || p === "/credits" || p === "/api/v1/credits") {
      if (req.method !== "GET") return toJson({ error: "method_not_allowed" }, req, env, 405);
      const email = getCallerEmail(req);
      if (!email) return toJson({ error: "missing_email" }, req, env, 400);
      const row = await getBalance(env, email);
      return toJson({ email, balance_credits: row.balance_credits, updated_at: row.updated_at, credits: row.balance_credits, updated: row.updated_at }, req, env, 200);
    }

    if (p === "/api/credits/adjust") {
      if (req.method !== "POST") return toJson({ error: "method_not_allowed" }, req, env, 405);
      const k = (req.headers.get("x-admin-key") || req.headers.get("x-admin-token") || "").trim();
      if (!env.ADMIN_API_KEY || k !== env.ADMIN_API_KEY) return toJson({ error: "unauthorized" }, req, env, 401);

      let body: any = {}; try { body = await req.json(); } catch { return toJson({ error: "invalid_json" }, req, env, 400); }
      const email = (body?.email || "").toString().toLowerCase().trim();
      if (!email) return toJson({ error: "missing_email" }, req, env, 400);

      const hasSet = typeof body?.set === "number";
      const hasDelta = typeof body?.delta === "number" && !Number.isNaN(body.delta);
      if (hasSet && hasDelta) return toJson({ error: "set_and_delta_conflict" }, req, env, 400);
      ...
      return toJson({ email, balance_credits: row.balance_credits, updated_at: row.updated_at }, req, env, 200);
    }
      ...
      try {
        if (req.method === "GET") {
          ...
          for (const k of list.keys) {
            ...
            if (v) { try { events.push(JSON.parse(v) as UsageEvent); } catch {} }
          }
          return toJson({ email, items: events, next_cursor: list.list_complete ? null : (list.cursor || null) }, req, env, 200);
        }
        if (req.method === "POST") {
          const email = getCallerEmail(req);
          if (!email) return toJson({ error: "missing_email" }, req, env, 400);
          let body: Partial<UsageEvent> = {};
          try { body = await req.json(); } catch { return toJson({ error: "invalid_json" }, req, env, 400); }
        ...
        return toJson({ events: [], error: "usage_failed", message: String(e?.message ?? e) }, req, env, 200);
      }
    }
    ...
    if (isJobsCollection(p)) {
      if (req.method === "GET") {
        ...
        const limit = Math.max(1, Math.min(Number(u.searchParams.get("limit") || "25"), 100));
        ...
        return toJson({ email, items, next_cursor: list.list_complete ? null : (list.cursor || null) }, req, env, 200);
      }
    ...
    if (p === "/api/jobs/run") {
      ...
      const type = String(body?.type || "si");
      const params = (body?.params && typeof body.params === "object") ? body.params : undefined;
      if (type !== "si") return toJson({ error: "unsupported_type", type }, req, env, 400);

      const job = await createJob(env, email, type, params);
      await processSiJob(env, job.id);
      const final = await getJob(env, job.id);
      if (!final) return toJson({ error: "job_missing_after_run", id: job.id }, req, env, 500);
    ...
    if (jobId) {
      if (req.method === "GET") {
        ...
        try { return toJson({ job: JSON.parse(raw) as Job }, req, env, 200); }
        catch { return toJson({ error: "corrupt" }, req, env, 500); }
      }
      if (req.method === "PATCH") {
        const raw = await env.KV_BILLING.get(jobRowKey(jobId));
        if (!raw) return toJson({ error: "not_found" }, req, env, 404);
        let job: Job;
        try { job = JSON.parse(raw) as Job; } catch { return toJson({ error: "corrupt" }, req, env, 500); }
        let body: any = {}; try { body = await req.json(); } catch { return toJson({ error: "invalid_json" }, req, env, 400); }
    ...
    if (p === "/api/ai/test") {
      ...
      try {
        ...
        return toJson({ ok: false, error: "ai_run_failed", detail: String(e?.message || e) }, req, env, 500);
      }
    }
    ...
    if (p === "/api/admin/cleanup") {
      ...
      try { body = await req.json<CleanupBody>(); } catch { return toJson({ error: "invalid_json" }, req, env, 400); }
      ...
      async function maybeDelete(key: string) { if (!dryRun) await env.KV_BILLING.delete(key); }
      ...
      if (kind === "usage" || kind === "both") {
        ...
        outer: while (true) {
          ...
          for (const k of list.keys) {
            ...
            let ts = 0;
            try { const row = JSON.parse(v) as UsageEvent; ts = Date.parse(row.created_at || ""); } catch {}
            if (ts && ts < cutoffTs) {
      ...
      if (kind === "jobs" || kind === "both") {
        ...
        outer2: while (true) {
          ...
          for (const k of list.keys) {
            ...
            let ts = 0;
            try { const job = JSON.parse(row) as Job; ts = Date.parse(job.created_at || ""); } catch {}
            if (ts && ts < cutoffTs) {
              await maybeDelete(k.name);
              res.deleted.job_index_keys++;
    ...
    if (p === "/api/upload/direct") {
      ...
      if (!bucket || typeof (bucket as any).put !== "function") return toJson({ error: "r2_not_bound" }, req, env, 500);
      ...
      if (!Number.isFinite(size) || size <= 0) return toJson({ error: "length_required" }, req, env, 411);
      if (size > maxBytes) return toJson({ error: "payload_too_large", max_bytes: maxBytes }, req, env, 413);

      const u = new URL(req.url);
      const filenameQ = (u.searchParams.get("filename") || "").trim();
      const base = (filenameQ || "").split(/[\\\/]/).pop() || "";
      const cleaned = base.replace(/[^\w.\-]+/g, "_").replace(/^\.+/, "").slice(0, 128) || "blob";
      ...
      const putRes = await bucket.put(key, req.body as any, {
        ...
        customMetadata: { uploader: email, route: "/api/upload/direct", original_filename: cleaned }
      });
    ...
    if (p === "/api/si/ask" || p === "/si/ask") {
      ...
      const balRow = await getBalance(env, email);
      if (!isGuest && (balRow.balance_credits ?? 0) <= 0) {
        return toJson({ error: "insufficient_credits", balance_credits: balRow.balance_credits }, req, env, 402);
      }
      ...
      try {
        ...
        ctx.waitUntil(
          appendUsage(env, email, "/api/si/ask",
            Number(out.tokens_in || 0),
            Number(out.tokens_out || 0),
            creditsUsed,
        ...
        if (updatedBalance !== null) extra["X-Credits-Balance"] = updatedBalance.toFixed(3);

        return toJson({ result: { content: out.text } }, req, env, 200, extra);
      } catch (e: any) {
        return toJson({ error: "si_ask_failed", detail: String(e?.message || e) }, req, env, 502);
      }
    }
    ...
    return toJson({ error: "internal_error", message: String(err?.message ?? err) }, req, env, 500);
  }
}

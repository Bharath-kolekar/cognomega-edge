...
```bash
curl -s https://api.cognomega.com/.well-known/jwks.json | jq
```
...
```bash
curl -sX POST https://api.cognomega.com/auth/guest | jq
# -> { "token": "...", "exp": ... }
```
...
* Ensure `ALLOWED_ORIGINS` includes your app origin (e.g., `https://app.cognomega.com`).
* Verify **preflight** reflects your custom headers. Test with explicit `Origin` and `Access-Control-Request-Headers` in curl.
